...
pub fn app_reducer(state: &mut AppState, action: AppAction) {
  match action {
    ...
    AppAction::SetTopology(topology) => {
      ...
      // Check if selected speaker/group should be cleared
      let mut should_clear_selection = false;

      // Check if selected speaker still exists in new topology
      if let Some(selected_speaker_uuid) = &state.selected_speaker_uuid {
        let speaker_exists = topology.groups.iter()
          .flat_map(|group| &group.speakers)
          .any(|speaker| &speaker.uuid == selected_speaker_uuid);
      ...
      // Check if selected group coordinator still exists and is still a coordinator
      if let Some(selected_group_uuid) = &state.selected_group_uuid {
        let group_coordinator_exists = topology.groups.iter()
          .flat_map(|group| &group.speakers)
          .any(|speaker| &speaker.uuid == selected_group_uuid && speaker.is_coordinator);
        
        if !group_coordinator_exists {
          should_clear_selection = true;
        }
      }
