...
use crate::streaming::types::{RawEvent, SubscriptionId};
...
impl CallbackServer {
    ...
    pub fn new(
        ...
        let port = port.ok_or_else(|| {
            SubscriptionError::CallbackServerError(format!(
    ...
    pub fn shutdown(&mut self) -> Result<(), SubscriptionError> {
        ...
        if let Some(handle) = self.server_handle.take() {
            handle.join().map_err(|_| {
                SubscriptionError::CallbackServerError("Failed to join server thread".to_string())
            })?;
        }
    ...
    pub fn register_subscription(
        ...
        self.event_router
            .register_subscription(subscription_id, callback_path)
    }
    ...
    fn get_local_ip() -> Option<String> {
        ...
        if let Ok(stream) =
            TcpStream::connect_timeout(&"8.8.8.8:80".parse().unwrap(), Duration::from_millis(1000))
        {
...
async fn handle_notify_request(
    ...
    event_router
        .handle_notify_request(path, headers, body)
        .await
}
...
impl EventRouter {
    ...
    pub async fn handle_notify_request(
        ...
        let subscription_id = match subscription_id {
            Some(id) => {
                println!(
                    "✅ Found subscription ID: {} for path: {}",
                    id, callback_path
                );
                id
            }
            None => {
                ...
                log::warn!(
                    "Received event for unknown callback path: {}",
                    callback_path
                );
        ...
        if !Self::validate_notify_headers(&headers) {
            log::warn!(
                "Invalid NOTIFY headers for subscription {}",
                subscription_id
            );
            return Ok(warp::reply::with_status(
                "Invalid headers",
        ...
        let event_xml = match String::from_utf8(body.to_vec()) {
            ...
            Err(_) => {
                log::warn!(
                    "Invalid UTF-8 in event body for subscription {}",
                    subscription_id
                );
                return Ok(warp::reply::with_status(
        ...
        log::debug!(
            "Successfully processed event for subscription {}",
            subscription_id
        );
